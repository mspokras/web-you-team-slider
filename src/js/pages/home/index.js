...
import { fetchDepartments, fetchDepartmentEmployees,  } from "../../shared/services/employees.js";
...
const entry = (context, module) => {
	...
	pageStore.on("update", ({ state, prevState }) => console.log(state, prevState));
...
function initEmloyeeView(store, events) {
	...
	store.on("update", ({ state, prevState }) => {
		if (state.selectedEmployeeIdx === prevState.selectedEmployeeIdx &&
				state.employees === prevState.employees) return;
